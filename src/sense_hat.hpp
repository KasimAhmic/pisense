...
public:
  SenseHat()
      : _bus("/dev/i2c-1"), _humiditySensor(this->_bus, "Humidity Sensor", hts221::ADDRESS),
  ...
  void testHardware() const {
    ...
    result &= SenseHat::checkHardwareId(this->_humiditySensor, hts221::reg::WHO_AM_I, hts221::DEVICE_ID);
    result &= SenseHat::checkHardwareId(this->_pressureSensor, lps25h::reg::WHO_AM_I, lps25h::DEVICE_ID);
    result &= SenseHat::checkHardwareId(this->_gyroAccelSensor, lsm9ds1::gyro::reg::WHO_AM_I, lsm9ds1::gyro::DEVICE_ID);
    result &= SenseHat::checkHardwareId(this->_magnetometerSensor, lsm9ds1::mag::reg::WHO_AM_I_M, lsm9ds1::mag::DEVICE_ID);

    if (result) {
      spdlog::info("✅ All SenseHat hardware components verified successfully");
    } else {
      spdlog::error("❌ One or more SenseHat hardware components failed verification. Please review the error messages "
                    "above for details.");
    }
  }

  float readTemperature() const { return this->readTemperature(false); }
  ...
  float readTemperature(bool asFahrenheit) const {
    ...
    if (temp0Raw == temp1Raw) {
      spdlog::error("Invalid temperature calibration data: T0_OUT and T1_OUT raw values are identical ({}). Cannot "
                    "perform interpolation.",
                    temp0Raw);
      return 0.0F;
    }

    const float temperature = tempCalPoint0 + (static_cast<float>(tempRaw - temp0Raw) * (tempCalPoint1 - tempCalPoint0) /
                                               static_cast<float>(temp1Raw - temp0Raw));
...
private:
  ...
  static bool checkHardwareId(const i2c::Device &device, uint8_t whoAmIReg, uint8_t expectedId) {
