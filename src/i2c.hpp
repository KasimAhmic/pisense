...
namespace i2c
{
  ...
  public:
    ...
    void setAddress(uint8_t addr) const
    {
      ...
      {
        spdlog::error("Failed to set I2C device address to 0x{:02X} on {}: fd={} | error={}", addr, this->_bus, this->_fd, strerror(errno));
  ...
  public:
    ...
    uint8_t readByte(uint8_t reg) const
    {
      ...
      {
        spdlog::error("Failed to write register address 0x{:02X} to I2C device {} at address 0x{:02X}: {}", reg, this->_name, this->_addr, strerror(errno));
      ...
      {
        spdlog::error("Failed to read byte from I2C device {} at address 0x{:02X}, register 0x{:02X}: {}", this->_name, this->_addr, reg, strerror(errno));
    ...
    void writeByte(uint8_t reg, uint8_t value) const
    {
      ...
      {
        spdlog::error("Failed to write byte to I2C device {} at address 0x{:02X}, register 0x{:02X}: {}", this->_name, this->_addr, reg, strerror(errno));
