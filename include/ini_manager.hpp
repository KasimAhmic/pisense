 ...
 * @author LineGM (https://github.com/LineGM/ini_manager/tree/main)
 *
...
namespace ini {
   ...
   */
  template <typename T>
  concept StreamExtractable = requires(std::istream &istream, T &value) { istream >> value; };
  ...
  public:
     ...
     */
    static auto from_file(const std::string &file_path) -> std::expected<ini_manager, std::error_code> {
      ini_manager manager;
     ...
     * or a `std::error_code` on failure.
     */
    static auto from_stream(std::istream &istream) -> std::expected<ini_manager, std::error_code> {
      ini_manager manager;
    ...
    public:
       ...
       */
      explicit section_accessor(std::shared_ptr<data_map> data, std::string section_name)
       ...
       */
      auto operator[](std::string_view key) -> std::string & { return (*m_data)[m_section_name][std::string{key}]; }
    ...
    public:
       ...
       * @param section_name The name of the section.
       */
      explicit const_section_accessor(std::shared_ptr<data_map> data, std::string section_name)
          : m_data(std::move(data)), m_section_name(std::move(section_name)) {}
      ...
      auto operator[](std::string_view key) const -> std::optional<std::string> {
        if (const auto it_section = (*m_data).find(m_section_name); it_section != (*m_data).cend()) {
          if (const auto it_key = it_section->second.find(std::string{key}); it_key != it_section->second.cend()) {
     ...
     */
    auto operator[](std::string_view section) -> section_accessor { return section_accessor{m_data, std::string{section}}; }
     ...
     */
    template <typename T> auto get_value(section section, key key) const noexcept -> std::optional<T> {
     ...
     */
    auto get_value_or_default(section section, key key, std::string default_value) const noexcept -> std::string {
     ...
     * @concept StreamExtractable<T>
     */
    template <typename T> auto get_value_or_default(section section, key key, T default_value) const noexcept -> T {
     ...
     */
    auto remove_section(section section) noexcept -> bool { return m_data->erase(std::string{section.value}) > 0; }
     ...
     * @return A `std::expected` indicating success or failure with an `std::error_code`.
     */
    auto add_from_file(const std::string &file_path) -> std::expected<void, std::error_code> {
     ...
     * @return A `std::expected` indicating success or failure with an `std::error_code`.
     */
    auto write_file(const std::string &file_path) const -> std::expected<void, std::error_code> {
    ...
    auto write_file() const -> std::expected<void, std::error_code> {
      if (m_file_path.empty()) {
        ...
        return std::unexpected(std::error_code(static_cast<int>(std::errc::operation_not_permitted), std::generic_category()));
      }
     ...
     */
    friend auto operator<<(std::ostream &ostream, const ini_manager &manager) -> std::ostream & {
    ...
    auto parse(std::istream &istream) -> std::expected<void, std::error_code> {
      ...
      while (std::getline(istream, line)) {
        ...
        if (line_view.starts_with('[') && line_view.ends_with(']')) {
          if (line_view.length() < 3) { // Handle empty section "[ ]" -> treat as unnamed or error?
                                        // Current behavior: treats as empty string section name ""
            current_section = "";
          } else {
            current_section = std::string{trim(line_view.substr(1, line_view.length() - 2))};
          }
        ...
        if (auto delimiter_pos = line_view.find('='); delimiter_pos != std::string_view::npos) {
